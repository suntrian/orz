 ...
 */
public abstract class TreeService<T extends TreeNode<T, ID>, ID extends Serializable> implements TreeInterface<T,ID> {
    ...
    public List<T> getChildList(List<T> list, T t) {
        ...
        while (it.hasNext()) {
             ...
             * 判断集合的父ID是否等于上一级的id
             */
            if (((TreeNode<T, ID>) child).getParentId() == ((TreeNode<T, ID>) t).getId()) {
